/* 
 * Maintainer = Po-Hsuan Huang
 * Edited = July 2026
 * Website = https://github.com/aben20807/bibtex-js-ext
 * Previous Website = https://github.com/pcooksey/bibtex-js
 * Credit = Henrik Mühe and Philip Cooksey
 *
 * Issues:
...
function BibtexParser() {
    ...
    this.getEntriesArray = function() {
        var entriesArray = [];
        for (var i = 0; i < this.entryOrder.length; i++) {
            var entryKey = this.entryOrder[i];
            if (this.entries.hasOwnProperty(entryKey)) {
                entriesArray.push(this.entries[entryKey]);
            }
        }
        return entriesArray;
    }
    ...
    this.entry_body = function(directive) {
        ...
        if (!this.entries.hasOwnProperty(this.currentEntry)) {
            this.entryOrder.push(this.currentEntry);
        }
...
function BibtexDisplay() {
    ...
    this.normalizeEntry = function(entry) {
        var archivePrefix = entry["ARCHIVEPREFIX"] || entry["EPRINTTYPE"] || "";
        var eprint = entry["EPRINT"] || "";

        if (archivePrefix && eprint && archivePrefix.toLowerCase() == "arxiv") {
            var normalizedEprint = eprint.replace(/^arxiv\s*:/i, "");
            var primaryClass = entry["PRIMARYCLASS"] || "";
            if (!entry["JOURNAL"]) {
                entry["JOURNAL"] = "arXiv:" + normalizedEprint;
                if (primaryClass) {
                    entry["JOURNAL"] += " [" + primaryClass + "]";
                }
            }
            if (!entry["URL"]) {
                entry["URL"] = "https://arxiv.org/abs/" + normalizedEprint;
            }
            if (!entry["BIBTEXTYPE"]) {
                entry["BIBTEXTYPE"] = "preprint";
            }
        }
    ...
    this.createTemplate = function(entry, output) {
        ...
        var findTopLevelOperator = function(expression, operator) {
            var depth = 0;
            for (var i = 0; i < expression.length; i++) {
                if (expression[i] == '(') {
                    ...
                    if (operator == '||' && expression.substring(i, i + 2) == '||') {
                        return i;
                    }
                    if (operator == 'and' &&
                        i > 0 && i + 4 < expression.length &&
                        expression.substring(i, i + 5).toLowerCase() == ' and ')
                    {
                        return i;
                    }
                }
        ...
        var splitTopLevel = function(expression, operator) {
            var parts = [];
            var start = 0;
            var index = findTopLevelOperator(expression, operator);
            while (index !== -1) {
                parts.push(expression.substring(start, index).trim());
                start = index + (operator == '||' ? 2 : 5);
                index = findTopLevelOperator(expression.substring(start), operator);
                if (index !== -1) {
                    index += start;
                }
            }
        ...
        var hasOuterParentheses = function(expression) {
        ...
        var evaluateClause = function(cls) {
            cls = cls.trim();
            var eqIndex = cls.indexOf('==');
            if (eqIndex !== -1) {
                var keyName = cls.substring(0, eqIndex).trim();
                var expectedValue = cls.substring(eqIndex + 2).trim();
                if (hasKey(keyName)) {
                    return self.fixValue(entry[keyName.toUpperCase()]) == expectedValue;
                }
                return false;
            }
            return hasKey(cls);
        ...
        var evaluateExpression = function(expression) {
            ...
            var orClauses = splitTopLevel(expression, '||');
            if (orClauses.length > 1) {
                for (var i = 0; i < orClauses.length; i++) {
                    if (evaluateExpression(orClauses[i])) {
                        return true;
                    }
                }
                return false;
            ...
            var andClauses = splitTopLevel(expression, 'and');
            if (andClauses.length > 1) {
                for (var j = 0; j < andClauses.length; j++) {
                    if (!evaluateExpression(andClauses[j])) {
                        return false;
                    }
                }
                return true;
            ...
            return evaluateClause(expression);
        };
        do {
            // find next if
            ...
            var keepIf = evaluateExpression(cond.attr('class').replace(/\bif\b/, '').trim());
            cond.removeAttr('class');
    ...
    this.displayBibtex = function(input, output) {
        ...
        var entriesArray = b.getEntriesArray();
        for (var i = 0; i < entriesArray.length; i++) {
            this.normalizeEntry(entriesArray[i]);
        }
        ...
        if (structure.length) {
            ...
            // iterate over bibTeX entries in source order
            for (var i = 0; i < entriesArray.length; i++) {
